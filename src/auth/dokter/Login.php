...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM dokter WHERE no_hp = ?";
    $result = mysqli_prepare($mysqli, $sql);
    mysqli_stmt_bind_param($result, "s", $no_hp);
    mysqli_stmt_execute($result);
    $CheckResult = mysqli_stmt_get_result($result);
    if ($CheckResult->num_rows <= 0) {
?>
        <script>
            alert(`Dokter belum terdaftar, silakan menghubungi admin!`)
        </script>
        ...
        if ($row['no_hp'] == $no_hp && $row['password'] == $password) {
            $_SESSION["login"] = true;
            $_SESSION["id"] = $row["id"];
        ...
        ?>
            ...
            <meta http-equiv='refresh' content='0;url=../../../src/pages/dokter/index.php'>
<?php
            die();
        }
        exit();
    ...
    <div class="login-wrapper">
        <div class="login-card">
            <div class="text-center mb-4">
                ...
                <h3 class="fw-bold gradient-text">Login Dokter</h3>
            </div>
            <form method="POST" action="">
                <div class="mb-4">
                    <input type="number" class="form-control mb-3" id="no_hp" name="no_hp" placeholder="No HP Dokter" required>
                    <input type="password" class="form-control" id="password" name="password" placeholder="Password" required>
                </div>
                <p class="text-muted small mb-4">Gunakan akun yang telah terdaftar untuk melakukan login</p>
                <div class="d-flex justify-content-between align-items-center">
                    <a href="#" class="link-custom">Buat Akun</a>
